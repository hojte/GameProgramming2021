...
void BirdGame::init() {
    ...
    for (int i=0;i<length;i++) {
        ...
        phys->initBox(b2_staticBody, s/physicsScale, {pos.x/physicsScale,pos.y/physicsScale}, 1);
    }
    ...
    for (int i=0;i<length;i++){
        ...
        auto phys = obj->addComponent<PhysicsComponent>();
        phys->initBox(b2_staticBody, s/physicsScale, glm::vec2(pos.x/physicsScale,pos.y/physicsScale), 1);

        auto coinObj = createGameObject();
        coinObj->name = "Coin";
        auto so_coin = coinObj->addComponent<SpriteComponent>();
        coinObj->setPosition(glm::vec2(pos.x+150,300));
        so_coin->setSprite(coinSprite);
        auto coinPhys = coinObj->addComponent<PhysicsComponent>();
        coinPhys->initCircle(b2_staticBody, 0.1f, coinObj->getPosition()/physicsScale, 1);
